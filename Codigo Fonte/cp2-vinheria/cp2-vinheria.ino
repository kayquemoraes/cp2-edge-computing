...
int ledAmareloPin  = 9;  // LED amarelo no pino 10
int ledVerdePin    = 8;  // LED verde no pino 11
int ldrPin         = A0;  // LDR no pino A0
int buzzerPin      =  13;   // Buzzer no pino 7
...
void ler_sensores(){
    ...
    int LDR_value = analogRead(ldrPin);  // Faz a leitura em bits (leitura de 10 bits)
    ...
    valorLDR = pow(10, (log10(resistance/R0) - A) / B);  // Converte a resistência em lux usando a equação matemática
    Serial.println("** valor em bits: ", LDR_value);
	Serial.print("Lux: ");   
	...
	float tensao = valorSensorTemp * 5 / 1024;
...
void verifica_sensores(){
	...
	else if(valorLDR > 280 && valorLDR <= 380){
		luz = MEIA_LUZ;
		Serial.println("Meia Luz");
	}
	else if(valorLDR > 380){
		luz = CLARO;
